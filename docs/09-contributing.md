...
3. Add a semantic binding in `tokens/semantic/core.json`.
...
2. Match the patterns in existing React Native components (`AppButton.tsx`, `AppCard.tsx`, `AppText.tsx`).
3. Use semantic tokens — never hex literals, never primitives directly.
4. Export from the relevant `index.ts`.
5. Update `docs/05-react-native.md` with API notes when needed.
6. Open a PR with screenshots / video.
...
- [ ] Mobile scope: is the change clearly aligned with Lira for Women app use cases?

## Code style

- TypeScript strict mode, no implicit any.
- Components accept `style` and follow existing RN composition patterns.
